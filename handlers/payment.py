...
async def send_subscription_menu_message(target: Message):
    """Отправить меню подписки (1 месяц — 5₽ ТЕСТ)."""
    await target.answer(
        "💎 <b>Подписка (ТЕСТОВЫЙ РЕЖИМ)</b>\n\n"
        "С подпиской вы получите:\n"
        "• ♾️ Безлимитные сообщения\n"
        "• ⚡ Быстрые ответы\n"
        "• 📸 Доступ к фотоальбомам\n"
        "• 💬 Приоритетная поддержка\n\n"
        "<i>1 месяц доступа — всего 5₽ (ТЕСТ)</i>",
        parse_mode="HTML",
    ...
    logger.info("Показано меню подписки (1м/5₽ ТЕСТ)")
...
async def handle_subscribe(callback: CallbackQuery):
    ...
    plans = {
        "1m": {"name": "1 месяц (ТЕСТ)", "price": 5, "days": 30}
    }
